"""
Manufacturer Form - Create and Edit Manufacturer (Supplier/Manufacturer)
"""
...
class ItemCompanyForm(tk.Frame):
    def __init__(self, parent, colors, item_company_handler, company_data, on_save, on_cancel):
        super().__init__(parent, bg=COLORS['background'])
        self.colors = COLORS
        self.item_company_handler = item_company_handler
    ...
    def create_widgets(self):
        ...
        if not self.is_edit_mode:
            self.create_field(form_container, "Company Code", "company_code",
                             placeholder="e.g., ABC1, XYZ2 (max 4 characters)", required=True, row=current_row)
        else:
            # Read-only in edit mode
            self.create_readonly_field(form_container, "Company Code", "company_code", row=current_row)
        current_row += 2

        # --- Company Name (*) ---
        self.create_field(form_container, "Company Name", "company_name",
                         placeholder="e.g., ABC Suppliers, XYZ Manufacturers", required=True, row=current_row)
        current_row += 2
        ...
        if not self.is_edit_mode:
            ...
            info_label = tk.Label(info_frame,
                                 text="ℹ Company Code must be alphanumeric and maximum 4 characters",
        ...
        save_btn = tk.Button(button_frame,
                            text="Create Manufacturer" if not self.is_edit_mode else "Update Manufacturer")
        save_btn.config(
            font=FONTS['button'],
    ...
    def create_field(self, parent, label_text, field_name, placeholder="", required=False, row=0):
        ...
        # Validation for company_code - max 4 characters
            ...
            if field_info.get('required'):
                if field_info['type'] == 'entry':
                    ...
                    if not value or value == placeholder:
                        ...
                        # Additional validation for company code
                        is_valid, message = self.item_company_handler.validate_company_code(value)
                        if not is_valid:
    ...
    def handle_save(self):
        ...
        # Save to database
        if self.is_edit_mode:
            success, message = self.item_company_handler.update_item_company(
                self.company_data['id'],
                data
            )
        else:
            success, message, company_id = self.item_company_handler.create_item_company(data)
