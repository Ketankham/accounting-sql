"""
Item Group Form - Create and Edit Item Group
"""
...
class ItemGroupForm(tk.Frame):
    def __init__(self, parent, colors, item_group_handler, item_group_data, on_save, on_cancel):
        super().__init__(parent, bg=COLORS['background'])
        self.colors = COLORS
        self.item_group_handler = item_group_handler
        self.item_group_data = item_group_data
        self.on_save_callback = on_save
        self.on_cancel_callback = on_cancel

        self.is_edit_mode = item_group_data is not None
    ...
    def create_widgets(self):
        ...
        if not self.is_edit_mode:
            self.create_field(form_container, "Item Group Code", "item_group_code",
                             placeholder="e.g., FG01, RM02 (max 4 characters)", required=True, row=current_row)
        else:
            # Read-only in edit mode
            self.create_readonly_field(form_container, "Item Group Code", "item_group_code", row=current_row)
        current_row += 2

        # --- Item Group Name (*) ---
        self.create_field(form_container, "Item Group Name", "item_group_name",
                         placeholder="e.g., Finished Goods, Raw Materials", required=True, row=current_row)
        current_row += 2
        ...
        if not self.is_edit_mode:
            ...
            info_label = tk.Label(info_frame,
                                 text="ℹ Item Group Code must be alphanumeric and maximum 4 characters",
        ...
        save_btn = tk.Button(button_frame,
                            text="Create Item Group" if not self.is_edit_mode else "Update Item Group")
        save_btn.config(
            font=FONTS['button'],
    ...
    def create_field(self, parent, label_text, field_name, placeholder="", required=False, row=0):
        ...
        # Validation for item_group_code - max 4 characters
        if field_name == 'item_group_code':
    ...
    def load_item_group_data(self):
        """Load existing item group data into form"""
        if not self.item_group_data:
            return

        for field_name, field_info in self.form_vars.items():
            value = self.item_group_data.get(field_name, '')
        ...
        for field_name, field_info in self.form_vars.items():
            if field_info.get('required'):
                if field_info['type'] == 'entry':
                    ...
                    elif field_name == 'item_group_code':
                        # Additional validation for item group code
                        is_valid, message = self.item_group_handler.validate_item_group_code(value)
                        if not is_valid:
    ...
    def handle_save(self):
        ...
        # Save to database
        if self.is_edit_mode:
            success, message = self.item_group_handler.update_item_group(
                self.item_group_data['id'],
                data
            )
        else:
            success, message, item_group_id = self.item_group_handler.create_item_group(data)
