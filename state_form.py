"""
State Form - Create and Edit State
"""
...
class StateForm(tk.Frame):
    def __init__(self, parent, colors, state_handler, state_data, on_save, on_cancel):
        ...
        self.is_edit_mode = state_data is not None
    ...
    def create_widgets(self):
        ...
        if not self.is_edit_mode:
            self.create_field(form_container, "State Code", "state_code",
                             placeholder="e.g., MH, DL, KA (max 4 characters)", required=True, row=current_row)
        else:
            # Read-only in edit mode
            self.create_readonly_field(form_container, "State Code", "state_code", row=current_row)
        current_row += 2

        # --- State Name (*) ---
        self.create_field(form_container, "State Name", "state_name",
                         placeholder="e.g., Maharashtra, Delhi", required=True, row=current_row)
        current_row += 2
        ...
        if not self.is_edit_mode:
            ...
            info_label = tk.Label(info_frame,
                                 text="ℹ State Code must be alphanumeric and maximum 4 characters",
        ...
        save_btn = tk.Button(button_frame,
                            text="Create State" if not self.is_edit_mode else "Update State")
        save_btn.config(
            font=FONTS['button'],
    ...
    def create_field(self, parent, label_text, field_name, placeholder="", required=False, row=0):
        ...
        # Validation for state_code - max 4 characters
    ...
    def load_state_data(self):
        """Load existing state data into form"""
        if not self.state_data:
            return

        for field_name, field_info in self.form_vars.items():
            value = self.state_data.get(field_name, '')
        ...
        for field_name, field_info in self.form_vars.items():
            if field_info.get('required'):
                if field_info['type'] == 'entry':
                    ...
                    elif field_name == 'state_code':
                        # Additional validation for state code
                        is_valid, message = self.state_handler.validate_state_code(value)
                        if not is_valid:
    ...
    def handle_save(self):
        ...
        # Save to database
        if self.is_edit_mode:
            success, message = self.state_handler.update_state(
                self.state_data['id'],
                data
            )
        else:
            success, message, state_id = self.state_handler.create_state(data)
