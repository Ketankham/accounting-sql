"""
UoM Management Screen - List, Create, Edit UoMs (Units of Measure)
"""
...
class UoMManagement(tk.Frame):
    def __init__(self, parent, colors):
        ...
        # Connect to database
        if not self.uom_handler.connect():
    ...
    def create_table_view(self):
        """Create the table view for UoMs"""
    ...
    def load_uoms(self):
        """Load UoMs from database and display in table"""
        # Clear existing rows
        for widget in self.table_body.winfo_children():
            widget.destroy()

        # Get UoMs
        uoms = self.uom_handler.get_all_uoms()

        if not uoms:
            # No UoMs found
            no_data_label = tk.Label(self.table_body,
                                    text="No UoMs found. Click 'Create New UoM' to add one.",
                                    font=FONTS['body'],
                                    bg=self.colors['background'],
        ...
        # Display UoMs
        for idx, uom in enumerate(uoms, 1):
            self.create_table_row(idx, uom)

    def create_table_row(self, sr_no, uom):
        ...
        # UoM Code
        code_label = tk.Label(row_frame, text=uom['uom_code'],
                             font=FONTS['body_bold'],
                             bg=row_bg,
        ...
        # UoM Name
        name_label = tk.Label(row_frame, text=uom['uom_name'],
                             font=FONTS['body'],
                             bg=row_bg,
                             fg=self.colors['text_primary'],
                             width=40,
        ...
        # Status
        status_color = self.colors['success'] if uom['status'] == 'Active' else self.colors['error']
        ...
        edit_btn = tk.Button(row_frame,
                            ...
                            padx=SPACING['sm'],
                            pady=SPACING['xs'],
                            command=lambda: self.show_edit_form(uom['id']))
    ...
    def show_create_form(self):
        """Show the create UoM form"""
        self.current_view = 'form'
        self.edit_uom_id = None

        # Hide create button and change title
        self.create_btn.pack_forget()
        self.title_label.config(text="Create New UoM")

        # Show form
        from uom_form import UoMForm
        self.show_form(None)

    def show_edit_form(self, uom_id):
        """Show the edit UoM form"""
        self.current_view = 'form'
        self.edit_uom_id = uom_id

        # Hide create button and change title
        self.create_btn.pack_forget()
        self.title_label.config(text="Edit UoM")

        # Get UoM data
        uom_data = self.uom_handler.get_uom_by_id(uom_id)

        if not uom_data:
            messagebox.showerror("Error", "UoM not found")
            self.show_list_view()
            return

        # Show form
        self.show_form(uom_data)

    def show_form(self, uom_data):
        """Show UoM form (create or edit)"""
        # Clear content container
        for widget in self.content_container.winfo_children():
            widget.destroy()
    ...
    def show_list_view(self):
        ...
        self.create_btn.pack(side=tk.RIGHT)
        self.title_label.config(text="UoM Master")
    ...
    def __del__(self):
        ...
        if hasattr(self, 'uom_handler'):
            self.uom_handler.disconnect()
