"""
City Form - Create and Edit City
"""
...
class CityForm(tk.Frame):
    def __init__(self, parent, colors, city_handler, city_data, on_save, on_cancel):
        ...
        self.is_edit_mode = city_data is not None
    ...
    def create_widgets(self):
        ...
        if not self.is_edit_mode:
            self.create_field(form_container, "City Code", "city_code",
                             placeholder="e.g., MUM, DEL (max 4 characters)", required=True, row=current_row)
        else:
            # Read-only in edit mode
            self.create_readonly_field(form_container, "City Code", "city_code", row=current_row)
        current_row += 2

        # --- City Name (*) ---
        self.create_field(form_container, "City Name", "city_name",
                         placeholder="e.g., Mumbai, Delhi", required=True, row=current_row)
        current_row += 2
        ...
        if not self.is_edit_mode:
            ...
            info_label = tk.Label(info_frame,
                                 text="ℹ City Code must be alphanumeric and maximum 4 characters",
        ...
        save_btn = tk.Button(button_frame,
                            text="Create City" if not self.is_edit_mode else "Update City")
        save_btn.config(
            font=FONTS['button'],
    ...
    def create_field(self, parent, label_text, field_name, placeholder="", required=False, row=0):
        ...
        # Validation for city_code - max 4 characters
        if field_name == 'city_code':
        ...
        for field_name, field_info in self.form_vars.items():
            if field_info.get('required'):
                if field_info['type'] == 'entry':
                    ...
                    elif field_name == 'city_code':
                        # Additional validation for city code
                        is_valid, message = self.city_handler.validate_city_code(value)
                        if not is_valid:
    ...
    def handle_save(self):
        ...
        # Save to database
        if self.is_edit_mode:
            success, message = self.city_handler.update_city(
                self.city_data['id'],
                data
            )
        else:
            success, message, city_id = self.city_handler.create_city(data)
