"""
Item Type Form - Create and Edit Item Type
"""
...
class ItemTypeForm(tk.Frame):
    def __init__(self, parent, colors, item_type_handler, item_type_data, on_save, on_cancel, on_delete=None):
        super().__init__(parent, bg=COLORS['background'])
        self.colors = COLORS
        self.item_type_handler = item_type_handler
        self.item_type_data = item_type_data
        self.on_save_callback = on_save
        self.on_cancel_callback = on_cancel
        self.on_delete_callback = on_delete

        self.is_edit_mode = item_type_data is not None
    ...
    def create_widgets(self):
        ...
        if not self.is_edit_mode:
            self.create_field(form_container, "Type Code", "type_code",
                             placeholder="e.g., FIN, RAW, PROD (max 4 characters)", required=True, row=current_row)
        else:
            # Read-only in edit mode
            self.create_readonly_field(form_container, "Type Code", "type_code", row=current_row)
        current_row += 2

        # --- Type Name (*) ---
        self.create_field(form_container, "Type Name", "type_name",
                         placeholder="e.g., Finished Goods, Raw Material", required=True, row=current_row)
        current_row += 2
        ...
        if not self.is_edit_mode:
            ...
            info_label = tk.Label(info_frame,
                                 text="ℹ Type Code must be alphanumeric, uppercase and maximum 4 characters",
        ...
        save_btn = tk.Button(button_frame,
                            text="Create Item Type" if not self.is_edit_mode else "Update Item Type")
        save_btn.config(
            font=FONTS['button'],
    ...
    def create_field(self, parent, label_text, field_name, placeholder="", required=False, row=0):
        ...
        # Validation for type_code - max 4 characters, uppercase
        if field_name == 'type_code':
            def validate_and_uppercase(new_value):
                # Allow only alphanumeric and max 4 characters
                if len(new_value) > 4:
                    return False
                if new_value and not new_value.replace('_', '').isalnum():
                    return False
                return True

            vcmd = (parent.register(validate_and_uppercase), '%P')
            entry.config(validate='key', validatecommand=vcmd)

            # Auto-uppercase on key release
            def to_upper(*args):
                value = var.get()
                if value:
                    var.set(value.upper())
            var.trace('w', to_upper)
    ...
    def load_item_type_data(self):
        """Load existing item type data into form"""
        if not self.item_type_data:
            return

        for field_name, field_info in self.form_vars.items():
            value = self.item_type_data.get(field_name, '')
            ...
            if field_info.get('required'):
                if field_info['type'] == 'entry':
                    ...
                    if not value or value == placeholder:
                        ...
                        # Additional validation for type code
                        is_valid, message = self.item_type_handler.validate_type_code(value)
                        if not is_valid:
    ...
    def handle_save(self):
        ...
        # Save to database
        if self.is_edit_mode:
            success, message = self.item_type_handler.update_item_type(
                self.item_type_data['id'],
                data
            )
        else:
            success, message, type_id = self.item_type_handler.create_item_type(data)
    ...
    def handle_delete(self):
        ...
        if self.on_delete_callback and self.is_edit_mode:
            item_type_id = self.item_type_data['id']
            type_name = self.item_type_data['type_name']
            self.on_delete_callback(item_type_id, type_name)
